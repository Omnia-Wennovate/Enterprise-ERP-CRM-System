          ...
          >
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
              <div className="flex items-center gap-3">
                <div className="p-2 rounded-xl bg-omnia-gold/10">
                  <FileText className="w-5 h-5 text-omnia-gold" />
                </div>
                <div>
                  <h2 className="text-lg font-bold text-foreground">New Quotation</h2>
                  <p className="text-xs text-muted-foreground">Create a professional travel quotation</p>
                </div>
              </div>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="flex-1 overflow-y-auto">
                    ...
                    <motion.div key="step0" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-5">
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        {QUOTE_TYPES.map((type) => (
                          <label key={type} className={`flex items-center gap-3 p-3 rounded-xl border-2 cursor-pointer transition-all ${quoteType === type ? 'border-omnia-gold bg-omnia-gold/10' : 'border-border hover:border-border'}`}>
                            <input type="radio" value={type} {...register('quote_type')} className="sr-only" />
                            <div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center flex-shrink-0 ${quoteType === type ? 'border-omnia-gold' : 'border-border'}`}>
                              {quoteType === type && <div className="w-2 h-2 rounded-full bg-omnia-gold/100" />}
                            </div>
                            <span className="text-sm font-medium text-foreground">{QUOTE_TYPE_LABELS[type]}</span>
                          </label>
                        ))}
                      </div>
                    ...
                    <motion.div key="step2" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-5">
                      ...
                      <div className="border border-border rounded-xl overflow-hidden">
                        ...
                        <div className="divide-y divide-gray-100">
                          {fields.map((field, idx) => (
                            <div key={field.id}
                              className="grid items-center px-3 py-2 gap-2 hover:bg-muted transition-colors"
                              style={{ gridTemplateColumns: '160px 1fr 60px 110px 70px 70px 110px 36px' }}>
                              <select
                                {...register(`items.${idx}.service`)}
                                className="text-xs px-2 py-1.5 border border-border rounded-lg bg-card focus:border-omnia-gold/60 outline-none"
                                onChange={(e) => {
                              ...
                              <input
                                ...
                                className="text-xs px-2 py-1.5 border border-border rounded-lg bg-card focus:border-omnia-gold/60 outline-none w-full"
                              />
                              <input
                                {...register(`items.${idx}.quantity`, { valueAsNumber: true })}
                                type="number" min="1"
                                className="text-xs px-2 py-1.5 border border-border rounded-lg bg-card focus:border-omnia-gold/60 outline-none text-center"
                                onChange={(e) => {
                              ...
                              <input
                                ...
                                className="text-xs px-2 py-1.5 border border-border rounded-lg bg-card focus:border-omnia-gold/60 outline-none text-right"
                                onChange={(e) => {
                              ...
                              />
                              <div className="text-xs font-semibold text-omnia-gold-dark text-right">
                        ...
                        <div className="px-3 py-2 border-t border-gray-100 bg-card">
                          <button type="button" onClick={addItem}
                            className="flex items-center gap-1.5 text-xs text-omnia-gold font-semibold hover:text-omnia-gold-dark transition-colors">
                      ...
                      <div className="flex justify-end">
                        <div className="w-64 bg-muted rounded-xl border border-border overflow-hidden">
                          <div className="px-4 py-2 flex justify-between text-sm text-muted-foreground">
                            <span>Subtotal</span>
                          ...
                          <div className="px-4 py-2 flex justify-between text-sm text-muted-foreground border-b border-border">
                            <span>Tax</span>
                            <span className="font-medium">{formatCurrency(watch('tax_amount'))}</span>
                          </div>
                          <div className="px-4 py-3 flex justify-between bg-omnia-gold text-primary-foreground">
                    ...
                    <motion.div key="step3" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-5">
                      ...
                      <div className="space-y-2">
                        ...
                        <div
                          ...
                          className="border-2 border-dashed border-border rounded-xl p-4 text-center hover:border-omnia-gold/40 hover:bg-omnia-gold/10/30 transition-all cursor-pointer">
                          <Upload className="w-6 h-6 text-gray-300 mx-auto mb-1" />
                          <p className="text-xs text-muted-foreground">Click to upload PDF, images, brochures</p>
                          <p className="text-xs text-muted-foreground mt-0.5">Files will be stored in Supabase Storage</p>
                        </div>
                        <input
                          ref={fileInputRef}
                          type="file"
                        ...
                        {uploadFiles.length > 0 && (
                          <ul className="space-y-1">
                            {uploadFiles.map((f, i) => (
                              <li key={i} className="flex items-center gap-2 text-xs text-muted-foreground bg-muted rounded-lg px-3 py-1.5">
                                <FileText className="w-3 h-3 text-omnia-gold" />
                                {f.name}
                                <button type="button" onClick={() => setUploadFiles((prev) => prev.filter((_, j) => j !== i))} className="ml-auto text-muted-foreground hover:text-red-500">
                                  <X className="w-3 h-3" />
                                </button>
                              </li>
                            ))}
              ...
              <div className="border-t border-gray-100 px-6 py-4 bg-card flex items-center justify-between gap-3">
                ...
                <div className="flex items-center gap-2 ml-auto">
                  ...
                  {step < STEPS.length - 1 ? (
                    <Button type="button" onClick={handleNext} className="bg-omnia-gold hover:bg-omnia-gold-dark text-primary-foreground gap-1.5">
                    ...
                    <Button
                      ...
                      className="bg-omnia-gold hover:bg-omnia-gold-dark text-primary-foreground gap-2 min-w-32">
                      {isSaving ? (
