    ...
    >
      ...
      <div className="flex items-start justify-between px-5 py-4 border-b border-gray-100 flex-shrink-0">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-xs font-mono font-bold text-teal-600">{quotation.quote_number}</span>
            <span className={`text-xs px-2 py-0.5 rounded-full font-semibold ${statusCfg.bg} ${statusCfg.text}`}>
              {QUOTATION_STATUS_LABELS[quotation.status]}
            </span>
          </div>
          <h3 className="font-bold text-gray-900 text-sm leading-tight truncate">{quotation.quote_title}</h3>
          <p className="text-xs text-gray-500 mt-0.5">{quotation.customer_name}</p>
        </div>
        <button onClick={onClose} className="p-1.5 rounded-lg text-gray-400 hover:text-gray-600 hover:bg-gray-100 transition-colors ml-2 flex-shrink-0">
          <X className="w-4 h-4" />
        </button>
      </div>
      ...
      <div className="grid grid-cols-3 gap-0 border-b border-gray-100 flex-shrink-0">
        <div className="px-4 py-3 text-center border-r border-gray-100">
          ...
          <p className="text-sm font-bold text-teal-700">{formatCurrency(quotation.grand_total)}</p>
        </div>
        <div className="px-4 py-3 text-center border-r border-gray-100">
          <p className="text-xs text-gray-500">Travelers</p>
          <p className="text-sm font-bold text-gray-800">{travelers}</p>
        </div>
        <div className="px-4 py-3 text-center">
          <p className="text-xs text-gray-500">Valid Until</p>
          <p className="text-xs font-bold text-gray-800">{formatDate(quotation.valid_until)}</p>
        </div>
      </div>
      ...
      <div className="px-4 py-2.5 border-b border-gray-100 flex items-center gap-2 flex-shrink-0">
        ...
        {statusUpdating && <Loader2 className="w-3 h-3 animate-spin text-teal-600" />}
      </div>
        ...
        {TABS.map((tab) => (
          <button key={tab} onClick={() => setActiveTab(tab)}
            className={`flex-1 text-xs font-semibold py-2.5 transition-colors border-b-2 ${
              activeTab === tab ? 'text-teal-600 border-teal-600' : 'text-gray-500 border-transparent hover:text-gray-700'
            }`}>
            {tab}
          </button>
        ))}
            ...
            <motion.div key="overview" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="p-5 space-y-5">
              ...
              <section>
                ...
                <div className="bg-gray-50 rounded-xl p-3 space-y-2">
                  <div className="flex items-center gap-2"><User className="w-3.5 h-3.5 text-gray-400 flex-shrink-0" /><span className="text-sm font-semibold text-gray-900">{quotation.customer_name}</span></div>
                  {quotation.company && <div className="flex items-center gap-2"><Building2 className="w-3.5 h-3.5 text-gray-400 flex-shrink-0" /><span className="text-xs text-gray-600">{quotation.company}</span></div>}
                  {quotation.contact_person && <div className="flex items-center gap-2"><User className="w-3.5 h-3.5 text-gray-400 flex-shrink-0" /><span className="text-xs text-gray-600">{quotation.contact_person}</span></div>}
                  {quotation.email && <div className="flex items-center gap-2"><Mail className="w-3.5 h-3.5 text-gray-400 flex-shrink-0" /><a href={`mailto:${quotation.email}`} className="text-xs text-teal-600 hover:underline">{quotation.email}</a></div>}
                  {quotation.phone && <div className="flex items-center gap-2"><Phone className="w-3.5 h-3.5 text-gray-400 flex-shrink-0" /><span className="text-xs text-gray-600">{quotation.phone}</span></div>}
                </div>
              </section>

              {/* Trip */}
              <section>
                <h4 className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-3">Trip</h4>
                <div className="bg-gray-50 rounded-xl p-3 space-y-2">
                  <div className="flex items-start gap-2"><MapPin className="w-3.5 h-3.5 text-teal-500 flex-shrink-0 mt-0.5" /><div><p className="text-sm font-semibold text-gray-900">{quotation.destination}</p>{quotation.country && <p className="text-xs text-gray-500">{quotation.city ? `${quotation.city}, ` : ''}{quotation.country}</p>}</div></div>
                  <div className="flex items-center gap-2"><Calendar className="w-3.5 h-3.5 text-gray-400" /><span className="text-xs text-gray-600">{formatDate(quotation.departure_date)} → {formatDate(quotation.return_date)}</span></div>
                  ...
                  {quotation.travel_type && <div className="flex items-center gap-2"><Tag className="w-3.5 h-3.5 text-gray-400" /><span className="text-xs text-gray-600 capitalize">{quotation.travel_type}</span></div>}
                </div>
              </section>
                ...
                <div className="space-y-2">
                  {[
                    ...
                    <div key={event!.label} className="flex items-center gap-2.5">
                      ...
                      <span className="text-xs text-gray-600 font-medium">{event!.label}</span>
                      <span className="text-xs text-gray-400 ml-auto">{formatDate(event!.date)}</span>
                    </div>
                  ))}
                </div>
              </section>
              ...
              {quotation.items.length === 0 ? (
                ...
                <div className="space-y-2">
                  {quotation.items.map((item, idx) => (
                    <div key={item.id ?? idx} className="bg-gray-50 rounded-xl p-3 border border-gray-100">
                      <div className="flex items-center justify-between mb-1">
                        <span className="text-xs font-bold text-teal-700 bg-teal-50 px-2 py-0.5 rounded-full">{item.service}</span>
            ...
            <motion.div key="terms" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="p-5 space-y-5">
              <div className="grid grid-cols-2 gap-3">
                <div className="bg-gray-50 rounded-xl p-3">
                  ...
                  <p className="text-sm font-semibold text-gray-800">{formatDate(quotation.quotation_date)}</p>
                </div>
                <div className="bg-gray-50 rounded-xl p-3">
                  <p className="text-xs text-gray-400 mb-1">Valid Until</p>
                  <p className="text-sm font-semibold text-gray-800">{formatDate(quotation.valid_until)}</p>
                </div>
              </div>
              {quotation.payment_terms && (
                <div>
                  <h4 className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Payment Terms</h4>
                  <p className="text-sm text-gray-700 bg-gray-50 rounded-xl p-3">{quotation.payment_terms}</p>
                </div>
              )}
              {quotation.cancellation_policy && (
                <div>
                  <h4 className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Cancellation Policy</h4>
                  <p className="text-sm text-gray-700 bg-gray-50 rounded-xl p-3">{quotation.cancellation_policy}</p>
                </div>
              )}
              {quotation.notes && (
                <div>
                  <h4 className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Notes</h4>
                  <p className="text-sm text-gray-700 bg-gray-50 rounded-xl p-3">{quotation.notes}</p>
                </div>
              )}
              {quotation.internal_notes && (
                <div>
                  <h4 className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Internal Notes</h4>
                  <p className="text-sm text-gray-700 bg-yellow-50 border border-yellow-200 rounded-xl p-3">{quotation.internal_notes}</p>
                </div>
              )}
            </motion.div>
          )}
                  ...
                  {quotation.attachment_urls.map((url, idx) => {
                    ...
                    return (
                      <a key={idx} href={url} target="_blank" rel="noopener noreferrer"
                        className="flex items-center gap-3 bg-gray-50 rounded-xl p-3 hover:bg-teal-50 hover:border-teal-200 border border-gray-100 transition-all group">
                        <Paperclip className="w-4 h-4 text-gray-400 group-hover:text-teal-500" />
      ...
      {quotation.creator_name && (
        <div className="px-5 py-3 border-t border-gray-100 flex-shrink-0">
          <p className="text-xs text-gray-400">Created by <strong className="text-gray-600">{quotation.creator_name}</strong> · {formatDate(quotation.created_at)}</p>
        </div>
      )}
    </motion.div>
  )
