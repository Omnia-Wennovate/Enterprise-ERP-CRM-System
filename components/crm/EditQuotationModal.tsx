                    ...
                    <motion.div key="e2" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-5">
                        ...
                        <div className="divide-y divide-gray-100">
                          {fields.map((field, idx) => (
                            <div key={field.id} className="grid items-center px-3 py-2 gap-2 hover:bg-gray-50"
                              style={{ gridTemplateColumns: '160px 1fr 60px 110px 70px 70px 110px 36px' }}>
                              <select {...register(`items.${idx}.service`)} className="text-xs px-2 py-1.5 border border-gray-200 rounded-lg bg-white focus:border-teal-400 outline-none">
                                {SERVICE_TYPES.map((s) => <option key={s} value={s}>{s}</option>)}
                              </select>
                              <input {...register(`items.${idx}.description`)} placeholder="Description" className="text-xs px-2 py-1.5 border border-gray-200 rounded-lg bg-white focus:border-teal-400 outline-none w-full" />
                              <input {...register(`items.${idx}.quantity`, { valueAsNumber: true })} type="number" min="1" className="text-xs px-2 py-1.5 border border-gray-200 rounded-lg text-center bg-white focus:border-teal-400 outline-none"
                                onChange={(e) => { setValue(`items.${idx}.quantity`, parseInt(e.target.value) || 1); setTimeout(() => updateItemTotal(idx), 0) }} />
                              <input {...register(`items.${idx}.unit_price`, { valueAsNumber: true })} type="number" min="0" step="0.01" className="text-xs px-2 py-1.5 border border-gray-200 rounded-lg text-right bg-white focus:border-teal-400 outline-none"
                                onChange={(e) => { setValue(`items.${idx}.unit_price`, parseFloat(e.target.value) || 0); setTimeout(() => updateItemTotal(idx), 0) }} />
                              <input {...register(`items.${idx}.discount`, { valueAsNumber: true })} type="number" min="0" max="100" step="0.1" className="text-xs px-2 py-1.5 border border-gray-200 rounded-lg text-center bg-white focus:border-teal-400 outline-none"
                                onChange={(e) => { setValue(`items.${idx}.discount`, parseFloat(e.target.value) || 0); setTimeout(() => updateItemTotal(idx), 0) }} />
                              <input {...register(`items.${idx}.tax_rate`, { valueAsNumber: true })} type="number" min="0" max="100" step="0.1" className="text-xs px-2 py-1.5 border border-gray-200 rounded-lg text-center bg-white focus:border-teal-400 outline-none"
                      ...
                      <div className="flex justify-end">
                        <div className="w-64 bg-gray-50 rounded-xl border border-gray-200 overflow-hidden">
                          ...
                          <div className="px-4 py-3 flex justify-between bg-teal-600 text-white"><span className="font-bold">Grand Total</span><span className="font-bold">{formatCurrency(watch('grand_total'))}</span></div>
                        </div>
                      </div>
                    </motion.div>
                  )}
              ...
              <div className="border-t border-gray-100 px-6 py-4 bg-white flex items-center justify-between">
                ...
                <div className="flex items-center gap-2 ml-auto">
                  ...
                  {step < EDIT_STEPS.length - 1 ? (
                    <Button type="button" onClick={handleNext} className="bg-teal-600 hover:bg-teal-700 text-white gap-1.5">Next<ChevronRight className="w-4 h-4" /></Button>
                  ) : (
                    <Button type="submit" disabled={isSaving || saveStatus === 'success'} className="bg-teal-600 hover:bg-teal-700 text-white gap-2 min-w-32">
