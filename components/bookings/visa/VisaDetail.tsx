...
export function VisaDetail({ visaId, onBack, onEdit }: VisaDetailProps) {
  ...
  if (!visa) {
    return <div className="text-center py-24 text-muted-foreground">Visa application not found</div>
  }
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div className="flex items-center justify-between bg-card rounded-xl shadow-sm border border-border p-4">
        <div className="flex items-center gap-4">
          <button onClick={onBack} className="p-2 hover:bg-muted rounded-lg text-muted-foreground transition-colors">
      ...
      <div className="flex flex-col md:flex-row gap-6">
        ...
        <div className="w-full md:w-64 flex-shrink-0">
          <div className="bg-card rounded-xl shadow-sm border border-border p-2 sticky top-6">
          ...
          <div className="bg-card rounded-xl shadow-sm border border-border p-6 min-h-[500px]">
            {activeTab === 'overview' && (
              <div className="space-y-8">
                {/* Applicant Info */}
                <section>
                  <h3 className="text-sm font-bold text-foreground uppercase tracking-wide border-b border-slate-100 pb-2 mb-4">Applicant Information</h3>
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-6">
                    <div>
                      <p className="text-xs text-muted-foreground font-medium">Full Name</p>
                      <p className="text-sm font-semibold text-foreground mt-1">{visa.traveler_first_name} {visa.traveler_last_name}</p>
                    </div>
                    <div>
                      <p className="text-xs text-muted-foreground font-medium">Nationality</p>
                      <p className="text-sm font-semibold text-foreground mt-1">{visa.traveler_nationality || 'N/A'}</p>
                    </div>
                    <div>
                      <p className="text-xs text-muted-foreground font-medium">Date of Birth</p>
                      <p className="text-sm font-semibold text-foreground mt-1">{visa.traveler_date_of_birth ? new Date(visa.traveler_date_of_birth).toLocaleDateString() : 'N/A'}</p>
                    </div>
                    <div>
                      <p className="text-xs text-muted-foreground font-medium">Passport Number</p>
                      <p className="text-sm font-semibold text-foreground mt-1">{visa.traveler_passport_number || 'N/A'}</p>
                    </div>
                    <div>
                      <p className="text-xs text-muted-foreground font-medium">Passport Expiry</p>
                      <p className="text-sm font-semibold text-foreground mt-1">{visa.traveler_passport_expiry ? new Date(visa.traveler_passport_expiry).toLocaleDateString() : 'N/A'}</p>
                    </div>
                  </div>
                </section>
                ...
                {/* Travel Info */}
                <section>
                  <h3 className="text-sm font-bold text-foreground uppercase tracking-wide border-b border-slate-100 pb-2 mb-4">Travel & Application Details</h3>
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-6">
                    <div>
                      <p className="text-xs text-muted-foreground font-medium">Destination</p>
                      <p className="text-sm font-semibold text-foreground mt-1">{visa.destination_country}</p>
                    </div>
                    <div>
                      <p className="text-xs text-muted-foreground font-medium">Visa Type</p>
                      <p className="text-sm font-semibold text-foreground mt-1 capitalize">{visa.visa_type.replace('_', ' ')}</p>
                    </div>
                    <div>
                      <p className="text-xs text-muted-foreground font-medium">Purpose</p>
                      <p className="text-sm font-semibold text-foreground mt-1">{visa.purpose_of_travel || 'N/A'}</p>
                    </div>
                    <div>
                      <p className="text-xs text-muted-foreground font-medium">Submission Date</p>
                      <p className="text-sm font-semibold text-foreground mt-1">{visa.submission_date ? new Date(visa.submission_date).toLocaleDateString() : 'Not submitted'}</p>
                    </div>
                    <div>
                      <p className="text-xs text-muted-foreground font-medium">Expected Decision</p>
                ...
                {visa.booking_id && (
                  <section>
                    ...
                    <div className="bg-muted/50 border border-border rounded-xl p-4 flex items-center justify-between">
                      ...
                      <button className="px-3 py-1.5 bg-card border border-border rounded-lg text-xs font-medium text-slate-700 hover:bg-muted/50">View Booking</button>
                    </div>
                  </section>
                )}
              </div>
