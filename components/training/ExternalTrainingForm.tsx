...
export function ExternalTrainingForm() {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      default: return <span className="px-2.5 py-1 rounded-full text-xs font-semibold bg-muted text-slate-700">{status}</span>
    }
  }
    ...
    <div className="space-y-6">
      ...
      {loading ? (
        <div className="space-y-4">
          {[1, 2].map(i => <div key={i} className="h-40 bg-card rounded-xl border border-border animate-pulse" />)}
        </div>
      ) : requests.length > 0 ? (
          ...
          {requests.map((req, idx) => (
            ...
            >
              ...
              <div className="flex-1">
                ...
                <h3 className="text-lg font-bold text-foreground mb-2">{req.course_name}</h3>
                <p className="text-sm text-muted-foreground mb-4 line-clamp-2">{req.justification}</p>

                <div className="flex flex-wrap gap-4 text-xs font-medium text-muted-foreground">
                  {req.provider && (
                    <span className="flex items-center gap-1.5"><Building className="w-3.5 h-3.5 text-muted-foreground" /> {req.provider}</span>
                  )}
                  {req.start_date && (
                    <span className="flex items-center gap-1.5"><Calendar className="w-3.5 h-3.5 text-muted-foreground" /> 
              ...
              {req.status === 'pending' && (
                <div className="flex md:flex-col justify-end gap-2 border-t md:border-t-0 md:border-l border-slate-100 pt-4 md:pt-0 md:pl-6 min-w-[140px]">
                  ...
                  <button className="flex-1 md:flex-none flex items-center justify-center gap-2 px-4 py-2 bg-card border border-rose-200 text-rose-600 rounded-lg hover:bg-rose-50 transition-colors text-sm font-medium">
        ...
        <div className="text-center py-20 bg-card rounded-xl border border-border/80">
          ...
          <h3 className="text-lg font-bold text-foreground mb-2">No External Requests</h3>
