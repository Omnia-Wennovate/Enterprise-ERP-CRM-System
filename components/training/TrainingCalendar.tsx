  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-card p-6 rounded-xl border border-border/80 shadow-sm">
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2 bg-muted rounded-lg p-1">
            <button onClick={prevMonth} className="p-1.5 hover:bg-card rounded-md text-muted-foreground">
              <ChevronLeft className="w-4 h-4" />
            </button>
            <span className="font-bold text-foreground w-32 text-center">{monthName}</span>
            <button onClick={nextMonth} className="p-1.5 hover:bg-card rounded-md text-muted-foreground">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        {/* Calendar Grid */}
        <div className="lg:col-span-3 bg-card rounded-xl border border-border/80 overflow-hidden shadow-sm">
          ...
          <div className="grid grid-cols-7">
            {days.map((day, i) => {
              ...
              return (
                <div 
                  key={i} 
                  className={`min-h-[120px] p-2 border-b border-r border-slate-100 relative ${!day ? 'bg-muted/50/50' : 'hover:bg-muted/50/50 transition-colors group'}`}
                >
                  {day && (
                    <>
        ...
        <div className="space-y-6">
          <div className="bg-card rounded-xl border border-border/80 p-5 shadow-sm">
            <h3 className="font-bold text-foreground mb-4 flex items-center justify-between">
              Upcoming Sessions
              <span className="text-xs bg-muted text-muted-foreground px-2 py-1 rounded-full font-semibold">{sessions.length}</span>
            </h3>
