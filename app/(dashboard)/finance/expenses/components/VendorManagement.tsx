  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search vendors..." className="px-4 py-2 border border-border rounded-lg bg-background text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-teal-500 w-72"/>
        <button onClick={openNew} className="flex items-center gap-2 px-4 py-2 bg-teal-600 hover:bg-teal-700 text-white rounded-lg text-sm font-semibold transition-colors" id="add-vendor-btn">
          ...
          <motion.div key={v.id} layout initial={{opacity:0,y:10}} animate={{opacity:1,y:0}} className="bg-card border border-border rounded-xl p-5 hover:shadow-md transition-shadow">
            <div className="flex items-start justify-between mb-3">
              ...
              <div className="flex gap-1">
                <button onClick={()=>openEdit(v)} className="p-1.5 rounded hover:bg-blue-50 text-blue-600 transition-colors"><Pencil size={13}/></button>
            ...
            <div className="border-t border-border mt-3 pt-3 grid grid-cols-3 gap-2 text-center">
              ...
              <div>
                ...
                <p className="text-sm font-bold text-teal-600">{fmt(v.total_paid ?? 0)}</p>
              </div>
              <div>
            ...
            <motion.div initial={{opacity:0,scale:0.95}} animate={{opacity:1,scale:1}} exit={{opacity:0,scale:0.95}}
              ...
              <form onSubmit={handleSubmit} className="p-6 space-y-3">
                <div>
                  ...
                  <input required value={form.name} onChange={e=>setForm(f=>({...f,name:e.target.value}))} className="w-full px-3 py-2.5 border border-border rounded-lg bg-background text-foreground text-sm focus:ring-2 focus:ring-teal-500 outline-none" placeholder="Company or vendor name"/>
                </div>
                  ...
                  {[['contact_person','Contact Person'],['phone','Phone'],['email','Email'],['tax_number','Tax Number']].map(([k,l])=>(
                    <div key={k}>
                      ...
                      <input value={form[k as keyof CreateVendorFormData]??''} onChange={e=>setForm(f=>({...f,[k]:e.target.value}))}
                        className="w-full px-3 py-2 border border-border rounded-lg bg-background text-foreground text-sm focus:ring-2 focus:ring-teal-500 outline-none"/>
                    </div>
                  ))}
                </div>
                <div>
                  ...
                  <input value={form.address??''} onChange={e=>setForm(f=>({...f,address:e.target.value}))}
                    className="w-full px-3 py-2.5 border border-border rounded-lg bg-background text-foreground text-sm focus:ring-2 focus:ring-teal-500 outline-none" placeholder="Full address"/>
                </div>
