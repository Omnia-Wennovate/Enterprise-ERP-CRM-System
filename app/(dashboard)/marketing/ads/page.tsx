        ...
        <main className="flex-1 overflow-y-auto p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            <div className="bg-card rounded-xl border border-border shadow-sm p-5">
              <div className="flex items-center gap-2 mb-2"><DollarSign size={16} className="text-blue-500" /><span className="text-xs text-muted-foreground">Total Budget</span></div>
              <p className="text-2xl font-bold text-foreground">${totalBudget.toLocaleString()}</p>
            </div>
            <div className="bg-card rounded-xl border border-border shadow-sm p-5">
              <div className="flex items-center gap-2 mb-2"><TrendingUp size={16} className="text-destructive" /><span className="text-xs text-muted-foreground">Total Spend</span></div>
              <p className="text-2xl font-bold text-foreground">${totalSpend.toLocaleString()}</p>
            </div>
            <div className="bg-card rounded-xl border border-border shadow-sm p-5">
              <div className="flex items-center gap-2 mb-3"><Eye size={16} className="text-primary" /><span className="text-xs text-muted-foreground">Budget Usage</span></div>
              <div className="w-full h-3 bg-background rounded-full overflow-hidden">
                <div className={`h-full rounded-full ${budgetUsagePercent > 90 ? 'bg-destructive' : budgetUsagePercent > 70 ? 'bg-[#F59E0B]' : 'bg-primary'}`} style={{ width: `${Math.min(100, budgetUsagePercent)}%` }} />
              </div>
              <p className="text-xs text-muted-foreground mt-1">{Math.round(budgetUsagePercent)}% used</p>
            </div>
          </div>

          {/* Platform Filter */}
          <div className="flex gap-2 mb-6">
            <button onClick={() => setFilterPlatform('all')} className={`px-4 py-2 rounded-lg text-sm font-medium ${filterPlatform === 'all' ? 'bg-primary text-primary-foreground' : 'bg-card text-muted-foreground border border-border'}`}>All ({ads.length})</button>
            {adPlatforms.map(p => {
              const count = ads.filter(a => a.platform === p).length
              return (
                <button key={p} onClick={() => setFilterPlatform(p)} className={`px-4 py-2 rounded-lg text-sm font-medium ${filterPlatform === p ? 'text-primary-foreground' : 'bg-card text-muted-foreground border border-border'}`} style={filterPlatform === p ? { backgroundColor: platformColors[p] } : {}}>
          ...
          {isLoading ? (
            ...
            <div className="bg-card rounded-xl border border-border shadow-sm p-12 text-center"><p className="text-muted-foreground">No advertisements found</p></div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {filtered.map(ad => {
                ...
                return (
                  <div key={ad.id} className="bg-card rounded-xl border border-border shadow-sm hover:shadow-md transition-all overflow-hidden">
                    ...
                    <div className="p-5">
                      ...
                      <div className="mb-4">
                        <div className="flex items-center justify-between text-xs mb-1">
                          <span className="text-muted-foreground">Budget: ${ad.budget.toLocaleString()}</span>
                      ...
                      <div className="flex items-center gap-2 pt-3 border-t border-border">
                        {ad.status === 'active' && <button onClick={() => handleStatusChange(ad.id, 'paused')} className="text-xs px-3 py-1.5 text-warning bg-[#F59E0B]/10 rounded-lg hover:bg-[#F59E0B]/20">Pause</button>}
                        ...
                        <button onClick={() => handleDelete(ad.id)} className="text-xs px-3 py-1.5 text-destructive bg-destructive/10 rounded-lg hover:bg-destructive/20 ml-auto">Delete</button>
                      </div>
                    </div>
                  </div>
                )
            ...
            <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
              <div className="bg-card rounded-xl shadow-xl w-full max-w-lg">
                <div className="flex items-center justify-between p-6 border-b border-border">
                  <h3 className="text-lg font-semibold text-foreground">Create New Ad</h3>
                  <button onClick={() => setShowModal(false)} className="text-muted-foreground hover:text-foreground"><X size={20} /></button>
                </div>
                <div className="p-6 space-y-4">
                  <div><label className="block text-sm font-medium text-foreground mb-1">Ad Name</label><input type="text" value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} className="w-full border border-border rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-primary" /></div>
                  <div className="grid grid-cols-2 gap-4">
                    <div><label className="block text-sm font-medium text-foreground mb-1">Platform</label><select value={form.platform} onChange={e => setForm({ ...form, platform: e.target.value })} className="w-full border border-border rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-primary">{adPlatforms.map(p => <option key={p} value={p}>{platformLabels[p]}</option>)}</select></div>
                  ...
                  <div><label className="block text-sm font-medium text-foreground mb-1">Target Audience</label><input type="text" value={form.target_audience} onChange={e => setForm({ ...form, target_audience: e.target.value })} className="w-full border border-border rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-primary" /></div>
                  <div className="grid grid-cols-2 gap-4">
                ...
                <div className="flex justify-end gap-3 p-6 border-t border-border">
                  ...
                  <button onClick={handleCreate} className="px-4 py-2 bg-primary text-primary-foreground text-sm font-medium rounded-lg hover:bg-primary/90">Create Ad</button>
                </div>
              </div>
            </div>
          )}
